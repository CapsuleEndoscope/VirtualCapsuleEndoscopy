...
public class ElectromagneticFieldControllerScript : MonoBehaviour {
    ...
    void AddRigidBody(GameObject obj, Rigidbody parentRigidbody, Collider parentCollider, int rigidbodyId, float poof) {
        ...
        if (parentCollider.gameObject.collider is BoxCollider) {
    ...
    public void ApplyForcesOnMagnetizable(bool applyExternalForce, bool applyDipoleForce, float strength, GameObject magnetizable, ref Dictionary<int,SpringJoint> sjToDipole, ref Dictionary<int,SpringJoint> sjFromDipole) {
        ...
        if (applyExternalForce) {
            ...
            Vector3 minBounds = magnetizable.collider.bounds.min;
            Vector3 maxBounds = magnetizable.collider.bounds.max;        
            ...
            while (!done) {
                ...
                if (magnetizable.collider.Raycast(ray, out hitInfo, (randomPos - outside).magnitude)) {
                    done = true;
                }
            }
            ...
            if (IsFinite(forceext)) {
                magnetizable.rigidbody.AddForce(forceext);
            }
            ...
            foreach (int rId in magneticDipolePosition.Keys) {
                foreach (int mId in magneticDipolePosition[rId].Keys) {
                    ...
                    if (!magneticDipoles[mId]) {
                        ...
                        float depth = 0.01f * Mathf.Min(magnetizable.collider.bounds.size.x, magnetizable.collider.bounds.size.y, magnetizable.collider.bounds.size.z);
                        ...
                        float m1 = magneticDipoles[mId].transform.parent.rigidbody.mass;
                        float m2 = magnetizable.rigidbody.mass;    
    ...
    void AddMagnetizableMaterialSprings(int magneticDipoleId, GameObject magnetizable, ref Dictionary<int,SpringJoint> sjToDipole, ref Dictionary<int,SpringJoint> sjFromDipole) {
        ...
        Rigidbody dipoleRigidbody = dipole.transform.parent.rigidbody;
        ...
        sjTo.connectedBody = magnetizable.rigidbody;
        sjTo.maxDistance = 0.01f * Mathf.Min(magnetizable.collider.bounds.size.x, magnetizable.collider.bounds.size.y, magnetizable.collider.bounds.size.z);
        sjTo.spring = 0.0f;
        toDipole.rigidbody.mass = 0.01f * magnetizable.rigidbody.mass;
        toDipole.rigidbody.useGravity = false;
        toDipole.rigidbody.isKinematic = true;
        ...
        sjFrom.spring = 0.0f;
        fromDipole.rigidbody.mass = 0.01f * dipoleRigidbody.mass;
        fromDipole.rigidbody.useGravity = false;
        fromDipole.rigidbody.isKinematic = true;
    ...
    Vector3 ClosestPointOnMagnetizable(int magneticDipoleId, GameObject magnetizable, float depth) {
        ...
        Vector3 notActuallyOnSurface = magnetizable.collider.ClosestPointOnBounds(dipolePosition);
        if (notActuallyOnSurface != dipolePosition) {
            Ray ray = new Ray(dipolePosition, (notActuallyOnSurface - dipolePosition).normalized);
            RaycastHit hitInfo;
            if (magnetizable.collider.Raycast(ray, out hitInfo, Mathf.Infinity)) {            
